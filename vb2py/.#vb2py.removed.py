...
class VBConverter(object):
	...
	# << class VBConverter methods >> (1 of 3)
	...
	# << class VBConverter methods >> (2 of 3)
	def doConversion(self, filename, callback=None):
		...
		if callback:
			callback("Done!", 100.0)
	# << class VBConverter methods >> (3 of 3)
...
#				converted to the proper (self.) style
			...
			#
			# These events should be stored in the control and then accessed within the loop on
			# a control by control basis
			events = (("%s_Click", "on_%s_mouseClick"),
					  ("%s_Change", "on_%s_textUpdate"),
					 )

			distinct_names = {}
			for event_name, new_name in events:
				for control in self.form._getControlsOfType():
					...
					for item in self.code_structure.locals:
						if event_name % name == item.identifier:
							...
							item.identifier = new_name % name
							event = vbparser.VBRenderDirect("event=None")
							item.parameters.append(event)
							item.scope = "Public"
...
#			same as the filename
		...
		try:
			...
			print "Failed during conversion of '%s'" % self.name
			self.form = None
		else:
...
class ClassParser(ModuleParser):
	...
	def getContainer(self):
		"""Return the container to use for code conversion"""
		return vbparser.VBClassModule(modulename=self.filename, classname=self.name)
...
# << VBConverter >> (13 of 14)
def usage(error=None):
	"""Print usage statement"""
	if error:
		print "\n\nInvalid option! (%s)" % error
	print "\nvb2py -chvs project.vpb destination\n\n" \
